...
async function main() {
  ...
  const SessionKeyOwnedValidator = "0x9Dd8825564b0D1C9a00b4C2166900412223Af132"
  ...
  const sesskeyWallet = new ethers.Wallet(sessionKeyPrive, provider);
  ...
  const call = {
    to: userAA1Addr,
    value: 0,
    data: KernelContract.interface.encodeFunctionData("execute", [receiveAddr, 13, "0x", 0]),
  };
  ...
  const permissions: Permission[] = [
    {
      ...
      valueLimit: 0,
      sig: getFunctionSelector("execute(address, uint256, bytes, Operation)"),
  ...
  // const sessionKeySigData = "0xc9c9ab0ba6f07b466255fba2c108c35b7f437978b6914d2dba3d5e43b2a1d5b256c0de549417443f8379140597d4965f99d2e7155d11af0a75b0e93c20f1f21b1b"

  let builder = kernel.execute(call)

  const op = await client.buildUserOperation(builder)
  const opHash = new UserOperationMiddlewareCtx(
    op,
    entryPoint,
    6480001000
  ).getUserOpHash()
  console.log("opHash:", opHash)
  const sessionKeySigData = await sesskeyWallet.signMessage(opHash);
